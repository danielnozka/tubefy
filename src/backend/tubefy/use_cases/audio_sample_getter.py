...
class AudioSampleGetter:
    ...
    def get(self, video_id: str) -> AudioOutput:
        ...
        database_audio_sample = self._audio_samples_persistence.get_audio_sample(video_id)

        if database_audio_sample is None:

            audio_sample = self._youtube_audio_downloader.download_audio_sample(
                video_id=video_id,
            ...
            database_audio_sample = self._audio_sample_adapter.adapt_to_persistence(audio_sample)
            self._audio_samples_persistence.add_audio_sample(database_audio_sample)

        else:

            audio_sample = self._audio_sample_adapter.adapt_to_domain(database_audio_sample)

            if not audio_sample.file_path.is_file():

                raise AudioFileNotFoundException(audio_sample.file_path)

        result = self._audio_sample_adapter.adapt_to_output_file(audio_sample)
