...
class SearchAdapter:
    ...
    def adapt_output(self, search_result_message: list[dict]) -> list[OutputSearchResult]:

        self._log.debug('Start [funcName]()')

        result: list[OutputSearchResult] = []

        for search_result_record in search_result_message:

            output_video_search_result = OutputSearchResult(video_id=search_result_record['id'],
